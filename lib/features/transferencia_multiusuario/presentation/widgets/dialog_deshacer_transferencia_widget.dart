...
import 'package:wms_app/features/transferencia_multiusuario/domain/usecases/undo_transferencia_claim_usecase.dart';
...
/// Espejo de DialogDeshacerRecepcionWidget. Confirmación para "deshacer" una
/// transferencia ya terminada (tab "Terminados") vía
/// POST /api/transfer/claim/{claimId}/undo. Requiere elegir una novedad
/// (misma lista de UserBloc.novedades) que explique el motivo.
class DialogDeshacerTransferenciaWidget extends StatefulWidget {
  const DialogDeshacerTransferenciaWidget({
    super.key,
  ...
  @override
  State<DialogDeshacerTransferenciaWidget> createState() =>
      _DialogDeshacerTransferenciaWidgetState();
}

class _DialogDeshacerTransferenciaWidgetState
    extends State<DialogDeshacerTransferenciaWidget> {
  Novedad? _selected;
  ...
  Future<void> _confirmar() async {
    ...
    final result = await getIt<UndoTransferenciaClaimUseCase>()(
      UndoTransferenciaClaimParams(
        claimId: claimId,
        observacion: _selected!.name,
      ),
    );
    ...
    return AlertDialog(
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            Text(
              '¿Deshacer la transferencia de "${widget.productName}"?\n\n'
              'Esto libera la cantidad ya almacenada para que pueda volver a '
