...
/// Réplica de RecepcionMultiusuarioNewLoteScreen para transferencia: listar
/// lotes existentes del producto o crear uno nuevo. Los lotes son un dato de
/// producto, no de sesión, así que reusa los mismos endpoints (GET
/// /api/lotes/{productId}, POST /api/create_lote) vía
/// TransferenciaMultiusuarioLoteBloc.
///
/// Al seleccionar/crear un lote (o cancelar) vuelve a
/// scan_product_screen.dart con `Navigator.pushReplacementNamed` (esta
/// pantalla reemplazó a esa al abrirse, no hay ruta a la que hacer pop).
class TransferenciaMultiusuarioNewLoteScreen extends StatefulWidget {
  const TransferenciaMultiusuarioNewLoteScreen({
    super.key,
  ...
  @override
  State<TransferenciaMultiusuarioNewLoteScreen> createState() =>
      _TransferenciaMultiusuarioNewLoteScreenState();
}

class _TransferenciaMultiusuarioNewLoteScreenState
    extends State<TransferenciaMultiusuarioNewLoteScreen>
  ...
  void _returnToScan({TransferenciaLoteProducto? lote}) {
  ...
  void initState() {
    ...
    if (productId != null) {
      context.read<TransferenciaMultiusuarioLoteBloc>().add(
        FetchTransferenciaLotesEvent(productId),
      );
    }
    _cargarPermisos();
  }
  ...
  void _submitCreateLote({bool priorityExpiration = false}) {
    ...
    final state = context.read<TransferenciaMultiusuarioLoteBloc>().state;
    final lotesCargados = state is TransferenciaLotesLoaded
        ? state.lotes
        : const [];
  ...
  Widget build(BuildContext context) {
    ...
    >(
      listener: (context, state) {
        if (state is CreateTransferenciaLoteLoading) {
          showLoadingDialog('Creando lote...');
        } else if (state is TransferenciaMultiusuarioLoteLoading) {
          showLoadingDialog('Cargando lotes...');
        } else {
          hideLoadingDialog();
        }
        if (state is CreateTransferenciaLoteSuccess) {
          _returnToScan(lote: state.lote);
        }
        if (state is CreateTransferenciaLoteNeedsConfirmation) {
        ...
        if (state is CreateTransferenciaLoteError) {
          showScrollableErrorDialog(state.message);
        }
      },
      child: Scaffold(
                ...
                if (_selectedIndex != null && _viewList)
                  Padding(
                    ...
                    child: ElevatedButton(
                      onPressed: () {
                        final state = context
                            .read<TransferenciaMultiusuarioLoteBloc>()
                            .state;
                        if (state is! TransferenciaLotesLoaded) return;
  ...
  List<TransferenciaLoteProducto> _filteredLotes(
    List<TransferenciaLoteProducto> lotes,
  ) {
  ...
  Widget _buildLotesList() {
    ...
    >(
      builder: (context, state) {
        // El loading de la carga (TransferenciaMultiusuarioLoteLoading) ya
        // lo cubre el diálogo del BlocListener — acá solo el flash inicial
        // antes de que se procese el FetchTransferenciaLotesEvent.
        if (state is TransferenciaMultiusuarioLoteInitial) {
          return const Center(child: CircularProgressIndicator());
        }
        if (state is TransferenciaMultiusuarioLoteError) {
          return Center(
            child: Text(
              state.message,
        ...
        final lotes = state is TransferenciaLotesLoaded
