...
class _DialogEditProductWidgetState extends State<DialogEditProductWidget> {
  ...
  double tolerance =
      0.000001; // Tolerancia para comparaciones de punto flotante

  @override
  void initState() {
    super.initState();
    if (widget.productsBatch.observation != null &&
        widget.productsBatch.observation!.isNotEmpty) {
      selectedNovedad = widget.productsBatch.observation;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final double quantityRequested =
        (widget.productsBatch.quantity ?? 0).toDouble();
    final double quantitySeparated =
        (widget.productsBatch.quantitySeparate ?? 0.0).toDouble();
    final double quantityRemaining = double.parse(
        (quantityRequested - quantitySeparated).toStringAsFixed(4));
            ...
            SizedBox(
              child: Column(
                children: [
                  RichText(
                    ...
                    text: TextSpan(children: [
                      TextSpan(
                          ...
                          style: TextStyle(fontSize: 13, color: black)),
                      TextSpan(
                        text: "${(quantityRemaining).toString()} ",
                        style: TextStyle(
                          fontSize: 13,
                  ...
                  // Dropdown de Novedad (visible si cantidad es menor a la requerida)
                  Visibility(
                    visible: (double.tryParse(batchBloc
                                .editProductController.text
                                .replaceAll(',', '.')) ??
                            quantityRemaining) <
                        quantityRemaining,
                    child: Card(
                      ...
                      child: Padding(
                        ...
                        child: DropdownButton<String>(
                          ...
                          items: batchBloc.novedades.map((Novedad item) {
                  ...
                  Padding(
                    ...
                    child: ElevatedButton(
                      onPressed: batchBloc.editProductController.text.isEmpty
                          ? null
                          : () async {
                              double cantidad = double.tryParse(batchBloc
                                      .editProductController.text
                                      .replaceAll(',', '.')) ??
                                  0.0;
                              ...
                              // Validamos usando la lógica centralizada
                                ...
                                final dynamic cantidadTotalRequest =
                                    (quantitySeparated + cantidad);

                                if (selectedNovedad != null &&
                                    cantidad < quantityRemaining) {
                                  batchBloc.add(UpdateNovedadProductEvent(
                                      selectedNovedad ?? '',
                      ...
                      child:
                          BlocBuilder<ClusterPickingBloc, ClusterPickingState>(
