...
class _ScanProductPickScreenState extends State<ScanProductPickScreen>
  ...
  bool validateScannedBarcode(String scannedBarcode,
      ProductsBatch currentProduct, PickScanBloc batchBloc, bool isProduct) {
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: BlocBuilder<PickScanBloc, PickScanState>(
        builder: (context, state) {
          final batchBloc = context.read<PickScanBloc>();

          int totalTasks =
              context.read<PickScanBloc>().filteredProducts.length;

          double progress = totalTasks > 0
              ? context.read<PickScanBloc>().filteredProducts.where((e) {
                    ...
                    BlocBuilder<ConnectionStatusCubit, ConnectionStatus>(
                      builder: (context, status) {
                        return Container(
                          ...
                          child:
                              BlocConsumer<PickScanBloc, PickScanState>(
                                    ...
                                    IconButton(
                                      onPressed: () {
                                        ...
                                        batchBloc.searchPickController.clear();
                                         context.read<PickingListBloc>().add(SearchPickListEvent(
                                          '',
                                          batchBloc.pickWithProducts.pick
                                                      ?.typePick ==
                                                  'pick'
                                        ...
                                        if (batchBloc.pickWithProducts.pick
                                                ?.typePick ==
                                            'pick') {
                                       context.read<PickingListBloc>().add(const FetchPicksFromDbEvent());
                                          Navigator.pushReplacementNamed(
                                              context, 'pick');
                                        } else {
                                          context.read<PickingComponentsBloc>().add(
                          ...
                          children: [
                            ...
                            LocationScannerWidget(
                              ...
                              locationDropdown: LocationDropdownWidget(
                                ...
                                // selección manual únicamente confirma esa, no
                                // debe listar todas las de origen del pick.
                                positionsOrigen: [
                            ...
                            ProductScannerWidget(
                              ...
                              productDropdown: ProductPickDropdownWidget(
                                ...
                                // Solo el producto actual: la selección manual
                                // únicamente confirma el producto en curso, no
                                // debe listar todos los pendientes del pick.
                                listOfProductsName: [
                                  batchBloc.currentProduct.productId.toString()
                                ],
  ...
  void _nextProduct(
      ProductsBatch currentProduct, PickScanBloc batchBloc) async {
  ...
  void validatePicking(PickScanBloc batchBloc, BuildContext context,
