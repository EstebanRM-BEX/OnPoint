...
class Tab3PedidoScreen extends StatefulWidget {
  const Tab3PedidoScreen({super.key});

  @override
  State<Tab3PedidoScreen> createState() => _Tab3PedidoScreenState();
}

class _Tab3PedidoScreenState extends State<Tab3PedidoScreen>
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<PackingPedidoBloc, PackingPedidoState>(
      listener: (context, state) {
        // Antes solo se escuchaba el error del delete: si el remanente
        // pendiente de una división ya no coincidía con la búsqueda del
        // repositorio (ver findAndAddQuantityAndDelete), o si el refresco
        // posterior (LoadPedidoAndProductsEvent) fallaba, "Aceptar" no
        // hacía absolutamente nada — sin error, sin confirmación.
        if (state is DeleteProductFromTemporaryPackageLoading) {
          showLoadingDialog('Deshaciendo producto...');
        } else {
          hideLoadingDialog();
        }
        if (state is DeleteProductFromTemporaryPackageOkState) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Producto devuelto a "Por hacer"')),
          );
        }
                      ...
                      ? const Center(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                'Intente con otro pedido o batch',
                                                                ...
                                                                child: RichText(
                                                                  ...
                                                                  text: TextSpan(
                                                                    children: [
                                                                      TextSpan(
                                                                        text:
                                                                            '¿Está seguro de eliminar el producto del paquete temporal: ',
                                                                        style: TextStyle(
                                                                          color:
                                                                              primaryColorApp,
                                                                          fontSize:
